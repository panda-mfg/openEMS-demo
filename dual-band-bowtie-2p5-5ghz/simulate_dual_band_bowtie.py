#!/usr/bin/env python3
"""Crossed dual-band bowtie for 2.5/5 GHz using the custom GPU openEMS build."""
...
TARGET_FREQUENCIES = np.array([2.5e9, 5.0e9])
FREQUENCY_MIN = 1.5e9
FREQUENCY_MAX = 6.2e9
REFERENCE_IMPEDANCE = 50.0
BOX_SIZE = np.array([180.0, 180.0, 130.0])


def triangle_points(apex, length, width, angle_deg):
    """Return a 2-D triangle extending from apex along angle_deg."""
    angle = np.deg2rad(angle_deg)
    axis = np.array([np.cos(angle), np.sin(angle)])
    normal = np.array([-axis[1], axis[0]])
    center = np.asarray(apex, dtype=float) + length * axis
    return np.vstack([
        np.asarray(apex, dtype=float),
        center + width / 2.0 * normal,
        center - width / 2.0 * normal,
    ])


def build_model(run_dir: Path, args):
    """Build and serialize the crossed dual-band model."""
    unit = 1e-3
    ...
    metal = csx.AddMetal("dual_band_bowtie_PEC")
    half_gap = args.feed_gap / 2.0
    pad_half_width = args.pad_width / 2.0
    left_pad = metal.AddBox(
        [-half_gap - args.pad_length, -pad_half_width, 0.0],
        [-half_gap, pad_half_width, 0.0],
        priority=15,
    )
    right_pad = metal.AddBox(
        [half_gap, -pad_half_width, 0.0],
        [half_gap + args.pad_length, pad_half_width, 0.0],
        priority=15,
    )

    left_apex = np.array([-half_gap - args.pad_length / 2.0, 0.0])
    right_apex = np.array([half_gap + args.pad_length / 2.0, 0.0])
    definitions = [
        ("low", args.low_length, args.low_width, args.branch_angle),
        ("high", args.high_length, args.high_width, -args.branch_angle),
    ]
    primitives = []
    all_points = [left_apex, right_apex]
    for _, length, width, right_angle in definitions:
        right_points = triangle_points(right_apex, length, width, right_angle)
        left_points = triangle_points(left_apex, length, width, right_angle + 180.0)
    ...
    mesh.AddLine("x", np.unique(np.r_[all_points[:, 0], -half_gap - args.pad_length,
                                      -half_gap, half_gap,
                                      half_gap + args.pad_length]))
    mesh.AddLine(
        "y",
        np.unique(np.r_[all_points[:, 1], -pad_half_width, 0.0, pad_half_width]),
    )
    fdtd.AddEdges2Grid(
        dirs="xy", primitives=primitives, metal_edge_res=maximum_cell / 2.0
    )

    port_start = [-half_gap, -args.port_width / 2.0, 0.0]
    port_stop = [half_gap, args.port_width / 2.0, 0.0]
    ...
    xml_path = run_dir / "dual_band_bowtie.xml"
    ...
    model_info = {
        ...
        "geometry_mm": {
            "low_band_branch": {
                "length": args.low_length,
                "width": args.low_width,
                "axis_angle_deg": args.branch_angle,
            },
            "high_band_branch": {
                "length": args.high_length,
                "width": args.high_width,
                "axis_angle_deg": -args.branch_angle,
            },
            "feed_gap": args.feed_gap,
            "port_width": args.port_width,
            "terminal_pad_length": args.pad_length,
            "terminal_pad_width": args.pad_width,
...
def postprocess(run_dir: Path, port, nf2ff, args):
    ...
    windows = [(1.8e9, 3.5e9), (3.7e9, 6.1e9)]
    keys = ["2p5ghz", "5ghz"]
    metrics = {
    ...
    axis.set(
        ...
        ylabel="$S_{11}$ (dB)",
        title="Crossed dual-band bowtie: input match",
    )
    ...
    if include_farfield:
        ...
        branch_phis = [args.branch_angle % 360.0, (-args.branch_angle) % 360.0]
        for key, target, branch_phi in zip(
            ...
            wanted_planes = [
                (branch_phi, "branch E-plane"),
                ((branch_phi + 90.0) % 360.0, "branch H-plane"),
            ]
...
def parse_args():
    ...
    parser.add_argument("--low-length", type=float, default=17.455)
    parser.add_argument("--low-width", type=float, default=26.210)
    parser.add_argument("--high-length", type=float, default=9.339)
    parser.add_argument("--high-width", type=float, default=14.019)
    parser.add_argument("--branch-angle", type=float, default=40.0)
    parser.add_argument("--feed-gap", type=float, default=0.55)
    parser.add_argument("--port-width", type=float, default=1.0)
    parser.add_argument("--pad-length", type=float, default=2.0)
    parser.add_argument("--pad-width", type=float, default=2.4)
    parser.add_argument(
        "--tuning-only",
        action="store_true",
