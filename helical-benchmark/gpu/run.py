...
Sim_Path = os.environ.get('OPENEMS_SIM_PATH',
                          os.path.join(os.path.dirname(__file__), 'results'))
force_re_sim = True
fixed_steps = int(os.environ.get('OPENEMS_FIXED_STEPS', '0'))
...
FDTD = openEMS(EndCriteria=0 if fixed_steps else 1e-4,
               **({'NrTS': fixed_steps} if fixed_steps else {}))
...
### Run the simulation
if os.environ.get('OPENEMS_XML_ONLY') == '1':
    os.makedirs(Sim_Path, exist_ok=True)
    FDTD.Write2XML(os.path.join(Sim_Path, 'helix.xml'))
    raise SystemExit(0)
...
if force_re_sim or not os.path.exists(os.path.join(Sim_Path, 'et')):
    run_options = {'engine': 'gpu', 'gpu_device': 0}
    if os.environ.get('OPENEMS_GPU_KERNEL'):
        run_options['gpu_kernel'] = os.environ['OPENEMS_GPU_KERNEL']
    if fixed_steps and os.environ.get('OPENEMS_USE_FIXED_FLAG') == '1':
        run_options['fixed_timesteps'] = True
    print('openEMS benchmark run options:', run_options)
    FDTD.Run(Sim_Path, cleanup=True, **run_options)

if os.environ.get('OPENEMS_SKIP_POSTPROCESS') == '1':
    raise SystemExit(0)
...
np.savez(os.path.join(os.path.dirname(__file__), 'metrics.npz'), freq=freq,
         Zin=Zin, s11=s11, Dmax_dB=Dmax_dB, Prad=nf2ff_res.Prad[0],
         efficiency=100*nf2ff_res.Prad[0]/np.interp(f0, freq, port.P_acc),
         theta_HPBW=theta_HPBW)
